...
onMounted(() => {
  ...
  const url = "/api/menu";
  page.httpOperations.get(url).then((response) => {
...
const toggle = async () => {
  ...
  page.container.isLoading = true;
  let url = "/api/menu";
  page.httpOperations.get(url).then((resp) => {
    page.container.isLoading = false;
    console.log(resp);
  });

  //store.useConfig().setLanguage("en");
  // page.success("111");
  // navigation("/inbound/1");
  //tagView.navigation("/inbound/1");
  //getAllViews();
};
...
<template>
  ...
  >
    <el-main>
      ...
      <Dialog
        v-model="inbound.visible"
        :title="inbound.title"
        @on-before-close="onModalClose"
      >
        <el-form-item label="Promotion name" label-width="160px">
          <el-input v-model="inbound.title" autocomplete="off" />
        </el-form-item>
      </Dialog>
    </el-main>
  </el-container>
